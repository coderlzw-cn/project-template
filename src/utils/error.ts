 ...
 */
export function getErrorCauseChain(
  error: unknown,
  maxDepth = 10,
): readonly Error[] {
...
function sanitizeErrorDetails(
  ...
  for (const key of getEnumerableKeys(details)) {
    ...
    result[key] = sanitizeErrorValue(
      readProperty(details, key),
      options,
      depth,
      seen,
    );
...
function sanitizeErrorValue(
  ...
  if (Array.isArray(value)) {
    return value.map(item =>
      sanitizeErrorValue(item, options, depth + 1, seen),
    );
  }
  ...
  const error =
    serialized.code === undefined
      ...
      : new AppError(serialized.message, {
          ...
          ...(serialized.statusCode === undefined
            ? {}
          ...
          ...(serialized.expose === undefined
            ? {}
            : { expose: serialized.expose }),
        });
...
function assertOptionalStatusCode(
