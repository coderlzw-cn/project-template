...
import { EventMessage } from '@myapp/common/src/constsnts/enum.constsnt';
console.log(EventMessage.BUILD_LOG);
...
async function bootstrap() {
  ...
  // 禁用默认的 JSON 解析器（使用拦截器处理）
  // httpAdapter.set('json replacer', (key: string, value: unknown) => (typeof value === 'bigint' ? value.toString() : value));
  app.use(
    helmet({
  ...
  Logger.log(`Environment: ${getNodeEnv()}`, 'Bootstrap');
