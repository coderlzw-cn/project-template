...
import { IoredisModule } from '@app/library/ioredis';
...
import { IoredisService } from '@app/library/ioredis';
...
export class MyService {
  ...
  async example() {
    ...
    const topPlayers = await this.redisService.zrange(
      'leaderboard',
      0,
      9,
      true,
...
const client = this.redisService.getClient();
// 使用 ioredis 的所有功能
await client.pipeline().set('key1', 'value1').set('key2', 'value2').exec();
```
